...
// Header-only: the DRM backend reads HELIX_EGL_VSYNC from here, and that backend is also
// linked into binaries that do not carry DisplayManager.
...
/// HELIX_EGL_VSYNC: `1` is on, `0` or unset is off, anything else is off with a warning.
inline bool egl_vsync_from_env() {
    const char* value = std::getenv("HELIX_EGL_VSYNC");
    ...
    spdlog::warn("[RefreshTiming] Ignoring HELIX_EGL_VSYNC='{}': expected 0 or 1", value);
    return false;
}
