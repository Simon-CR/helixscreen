...
@test "the tree sits at its ratchet baseline, not under it" {
    # This branch carries pre-existing inert assertions, so the gate runs with
    # a baseline rather than at zero. Pinning equality keeps the baseline
    # honest: it fails if the tree drifts up, and also if it drifts down
    # without the baseline following it.
    local baseline
    baseline=$(sed -n 's/.*check_bats_inert_assertions\.py --max-allowed \([0-9]*\).*/\1/p' \
        "$BATS_TEST_DIRNAME/../../scripts/quality-checks.sh" | head -1)
    [ -n "$baseline" ]

    run python3 "$GATE" --max-allowed "$baseline"
    [ "$status" -eq 0 ]
    contains "== baseline ($baseline)" "$output"
}
