...
PageSetChange page_set_change_of(int page_count, bool page_added, int removed_page,
                                 int focus_page) {
...
TEST_CASE("a page-set rebuild comes up on the page on screen and slides once to its focus",
    ...
    const LandingRow rows[] = {
        {"a drop onto the next-page slot", page_set_change_of(2, true, -1, 2), 2, 2, 2},
        {"a drop onto the next-page slot that empties its origin page",
         page_set_change_of(2, true, 1, 2), 2, 1, 1},
        {"a drop past the last page's border that keeps its origin page",
         page_set_change_of(1, true, -1, 1), 0, 0, 1},
        {"a drop past the last page's border that empties its origin page",
         page_set_change_of(2, true, 1, 2), 1, 1, 1},
        {"a move that empties a page before its landing page", page_set_change_of(4, false, 1, 2),
         2, 1, 1},
        {"a move that empties a page after its landing page", page_set_change_of(2, false, 1, 0), 0,
         0, 0},
        {"removing the last widget of the last of three pages", page_set_change_of(3, false, 2, 2),
         2, 1, 1},
        {"removing the last widget of the middle of three pages",
         page_set_change_of(3, false, 1, 1), 1, 1, 1},
        {"the delete-page button on the last of two pages", page_set_change_of(2, false, 1, 1), 1,
         0, 0},
        {"a page on screen past the removed page comes up renumbered and slides to the focus",
         page_set_change_of(4, false, 1, 1), 3, 2, 1},
        {"a removed page on screen gives way to a focus on another page",
         page_set_change_of(4, false, 1, 3), 1, 2, 2},
        {"the next-page slot on screen with no page added there",
         page_set_change_of(2, false, -1, 1), 2, 1, 1},
    };
