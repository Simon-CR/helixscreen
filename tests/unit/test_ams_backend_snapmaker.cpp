...
// ============================================================================
// The auto-mirror and the lane's own declarations (prestonbrown/helixscreen#1654)
// ============================================================================
//
// OverwriteAlways hands every unlocked colour and material in the stored
// override to firmware. A declaring lane record naming a different value
// outranks the reading the frame files, so the lane gives that field up too,
// or the screen paints a value the override has stopped holding and the next
// restart reloads the other one.
...
TEST_CASE_METHOD(SnapmakerFixture,
                 "Snapmaker firmware restating a linked lane's colour takes it from the Spoolman "
                 "record",
                 "[ams][snapmaker][lane][1654]") {
    SnapmakerLaneRig rig("mirror_retracts_linked");
    ...
    CHECK(info.color_rgb == 0x112233u);
    CHECK(info.material == "PETG");
    // Only the restated fields go: the binding and the server's brand stand.
    CHECK(info.spoolman_id == 42);
    CHECK(info.brand == "Polymaker");
    ...
    CHECK_FALSE(sources.spoolman->material.has_value());
    CHECK(sources.spoolman->spoolman_id == 42);
    CHECK(sources.spoolman->brand == "Polymaker");
}

TEST_CASE_METHOD(SnapmakerFixture,
                 "Snapmaker a frame repeating what a linked lane declares retracts nothing",
                 "[ams][snapmaker][lane][1654]") {
    // The mirror runs on every frame. A frame whose firmware values agree with
    // the lane's declaration re-authors nothing, so the lane keeps its records.
...
TEST_CASE_METHOD(SnapmakerFixture,
                 "Snapmaker a Spoolman poll re-filing the old colour gives way on the next frame",
                 "[ams][snapmaker][lane][1654]") {
    // The weight poll re-files the spool's whole record, colour and material
    // included, while the override already holds firmware's values. The next
    // frame finds the lane contradicting the override and retracts again.
    SnapmakerLaneRig rig("mirror_after_spoolman_poll");
    AmsBackendSnapmaker& backend = *rig.backend_reg;

    SnapmakerTestAccess::seed_override(backend, 0, linked_to_spool_42());
    SnapmakerTestAccess::handle_status(backend, seated_tag_frame(0xFF112233u, "PETG"));

    helix::test::spool_states(backend, 0, spool_42());
    {
        const auto refiled = helix::ams::lane_sources(rig.backend_reg.lane(0));
        REQUIRE(refiled.spoolman.has_value());
        REQUIRE(refiled.spoolman->color_rgb == 0xFF5500u);
    }
...
TEST_CASE_METHOD(SnapmakerFixture,
                 "Snapmaker firmware restating a colour picked before a link takes it from the "
                 "user's record",
                 "[ams][snapmaker][lane][1654]") {
    // A link carrying a different colour releases the pick's lock, while the
    // user's record, which the link amends with the binding alone, still names
    // the pick.
    SnapmakerLaneRig rig("mirror_retracts_unlocked_pick");
    AmsBackendSnapmaker& backend = *rig.backend_reg;

    SnapmakerTestAccess::handle_status(backend, seated_tag_frame(0xFF334455u, "PLA"));

    SlotInfo pick = backend.get_slot_info(0);
    pick.color_rgb = 0xFFFFFF;
    helix::test::edit_slot_as_user(backend, 0, pick);
    {
        const auto picked = SnapmakerTestAccess::get_override(backend, 0);
        REQUIRE(picked.has_value());
        REQUIRE(picked->user_locked_color);
    }

    SlotInfo linked = backend.get_slot_info(0);
    linked.spoolman_id = 42;
    linked.brand = "Polymaker";
    linked.color_rgb = 0xFF5500;
    helix::test::edit_slot_as_user(backend, 0, linked);
    helix::test::spool_states(backend, 0, spool_42());
    {
        const auto relinked = SnapmakerTestAccess::get_override(backend, 0);
        REQUIRE(relinked.has_value());
        REQUIRE_FALSE(relinked->user_locked_color);
