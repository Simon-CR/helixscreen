...
| `check_old_toolchain.py` | Re-parses every `src/` translation unit's own flags from `compile_commands.json` with `-fsyntax-only` against an older compiler (default `g++-10`, matching the AD5M/Pi/K1/K2/CC1 cross toolchains' libstdc++), catching a construct the CI runner's newer clang accepts and an older libstdc++ rejects (`std::from_chars<float>`, absent before GCC 11) before it reaches a cross build or a device. Run from `compile-ubuntu` in `.github/workflows/build.yml`, right after the native build produces the compile database. `--compiler`, `--jobs`, or specific files to narrow it |
