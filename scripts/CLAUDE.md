...
| `version-compare.sh` | **Semver precedence for shell consumers** - `version-compare.sh A B` prints `-1`/`0`/`1`, and a version it cannot parse is a non-zero exit rather than a verdict. The `Guard against channel downgrade` step in `.github/workflows/release.yml` asks it whether publishing moves a channel's manifest forward; `sort -V` cannot answer, it ranks `1.1.0` below `1.1.0-beta.1`. Ordering cases live in `tests/fixtures/version_precedence.txt`, shared with `src/util/version.cpp`; `tests/shell/test_version_compare.bats` asserts the whole matrix |
| `android-version-code.sh` | **The single definition of the Android versionCode** - `(major*1000000 + minor*1000 + patch)*100 + ordinal` from `VERSION.txt`, where the ordinal encodes the prerelease suffix (`-alpha.N`/`-beta.N`/`-rc.N`, 99 for a release), plus the lane-overflow guard. `android/app/build.gradle`, `generate-whatsnew.sh` and `.github/workflows/release.yml` all call it; `tests/shell/test_android_version_code.bats` fails on a second copy of the arithmetic |
...
| `run_suite_report.sh` | Produce the shared `--success` Catch2 report that `check_vacuous_tests.py` and `check_test_order_dependence.py` both read (`make suite-report`, and the nightly's own step). One unsharded process with its stdout discarded, so the report's byte count is the only progress signal there is: the script prints which case the run is inside, and stops it - naming that case - when the report stops growing (`SUITE_REPORT_STALL_SECS`) or the wall clock runs out (`SUITE_REPORT_MAX_SECS`). A red suite still yields a usable report. Meta-tests: `tests/shell/test_suite_report_guard.bats` |
...
| `check_cjk_font_staleness.sh` + `check_cjk_font_coverage.py` | Fail if the runtime CJK font (`assets/fonts/cjk/*.bin`) is missing a codepoint the translations need. Staleness diffs the needed set — extracted by `translations/cjk_charset.py`, the same extractor `regen_text_fonts.sh` bakes from — against `.cjk_codepoints.manifest`; coverage parses the .bin cmap tables themselves (`lv_binfont_loader.c` layout) so a stale bake cannot hide behind a freshly written manifest. A missing glyph renders as tofu in zh/ja with no other symptom. Both run from `qc_cjk_fonts` on tracked files only - no `.otf` source fonts needed, so they hold in every clone and CI runner. Meta-tests: `tests/shell/test_cjk_font_staleness_gate.bats`, `tests/shell/test_cjk_font_coverage_gate.bats` |
...
| `check_printer_image_invalidation.py` | Fail if code outside `src/system/` calls `invalidate_printer_image_cache()`: entries are named with the source's mtime and size, so a refresh has nothing stale to drop and every entry deleted costs a decode-and-resize to rebuild. |
...
| `check_json_dump_utf8.py` | Reject a bare `.dump()` on a save path — a stream write or an HTTP body (#1493). nlohmann's default `error_handler_t::strict` throws on the invalid UTF-8 that SSIDs, printer/tool names, file names and gcode responses routinely carry, costing the whole document; `helix::json_util::safe_dump()` replaces the bytes instead. Silent on log arguments. Opt out `// JSON_DUMP_OK: reason` |
...
| `apply_submodule_patch.sh` | The one apply verdict every `mk/patches.mk` stanza routes through: apply, already-applied (reverse check), or refuse — a bare `git apply --check` else-branch cannot tell "already applied" from "will never apply", and shared-file patches can fail both checks on a correctly patched tree. That neither-branch is triaged by the marker table (`check_patch_markers.py --only`): marker present reads as applied (the routine sibling-moved-context case), marker absent names the missing effect, no row keeps the hedge. Incremental runs warn; `make reapply-patches` sets `HELIX_PATCHES_FROM_CLEAN=1`, where a patch that will not take its apply branch is fatal. Meta-tests: `tests/shell/test_apply_submodule_patch.bats` |
...
| `check_patch_markers.py` | Fail the build when a wired patch's effect is absent from the checkout: `mk/patch-markers.tsv` names one line each patch adds (or removes) that upstream never contained, and every build greps for it — a text search needing no git, so it holds in docker trees rsynced from worktrees. Also fails a changed patch file (recorded hash) and a wired stanza with no row, so new coverage cannot pass vacuously. `gen_patch_markers.py` rederives the table (`make regen-patch-markers`), refusing a patch whose only candidates another patch could satisfy. `--only PATCH` answers presence for one patch (exit 0/1/2) for the apply helper's warn triage. Meta-tests: `tests/shell/test_patch_markers.bats` |
| `check_workflow_submodules.py` | Fail if a workflow job runs the shell suite (`make test-shell` or `bats tests/shell/`) without both `./.github/actions/init-submodules` and `make apply-patches`; `test_lvgl_event_code_gate.bats` needs a patched checkout. Also fails any `make apply-patches` step missing `HELIX_PATCHES_FROM_CLEAN=1`: a CI runner is a fresh clone, where a drifted patch is dead and must fail the job, not warn - and a make inside `docker run` must forward the flag with `-e`, since workflow env stops at the container boundary; a no-value `-e` additionally needs the env set, because it reads its value from the runner. Gated as `qc_workflow_submodules`; meta-tests: `tests/shell/test_workflow_submodules_gate.bats` |
...
| `check_rotation_cache_order.py` | Fail if a `DisplayManager` function caches `m_width`/`m_height` from the LVGL resolution before `set_display_rotation()` settles: a scanout plane owning 90/270 un-swaps the resolution and the cache would record a value the display no longer has (#1587). Source-order lint because `apply_rotation`'s body is `#ifndef HELIX_DISPLAY_SDL`-guarded out of the test binary. Fails closed both ways: each cache write consumes the nearest unconsumed settle before it, so a third wrongly ordered pair cannot borrow an earlier one, and a `GUARDED_FUNCTIONS` census asserts that `init`, `apply_rotation` and `run_rotation_probe` are still defined and still perform their recorded number of pairs - a rename, a wrapper around the settle call or a cache write extracted into a helper is a failure, not a pass. Bodies are brace-matched with comments, literals and preprocessor lines blanked. A cache refresh with no settle call in the function (the external-resize catch-up) is legal; opt a settle with no cache read after it out with `// ROTATION_CACHE_OK: reason`. `--rules-only` drops the census, for the meta-tests. |
| `check_platform_manifest.py` | **Advisory.** Report where `assets/config/platforms.json` and the consumers that derive from it disagree: a `package-*` target wired to another platform's `gen-splash-3d-*`, a size class hardcoded back into `mk/images.mk`, an install root the discovery lists in `common.sh` / `platform.sh` / `log_collector.cpp` / `update_checker.cpp` / `debug_bundle_collector.cpp` have not been taught, a panel with no hardware behind it. `--strict` turns findings into a failure, `--root` points it at another tree. The derived ladder is shared with `platform_manifest.py` and cross-checked against the compiled copy by `tests/unit/test_prerender_size_class.cpp` |
| `platform_manifest.py` | Read `assets/config/platforms.json` and answer derived questions about a platform - effective resolution after rotation, which splash classes its panel can select, printer image size. `mk/images.mk` calls it so `gen-splash-3d-<platform>` carries no resolution knowledge of its own; an unknown platform is a non-zero exit, never a silent fall-through to "generate everything". `prune-assets <platform> <staged-root>` strips a staged release down to what its panel can display and its build can play - 27 MB of the 42 MB `assets/images` on a real K2 package, plus `assets/sounds` where no tracker is compiled in - and is called from `release-clean-assets` in `mk/cross.mk`; it keeps the `generic-corexy` fallback, and keeps the source PNGs whenever any printer lacks a render at the size being kept |
| `audit_codebase.sh` | Check for coding standard violations. `--strict` for CI |
| `check_old_toolchain.py` | Re-parses every `src/` translation unit's own flags from `compile_commands.json` with `-fsyntax-only` against an older compiler (default `g++-10`, matching the AD5M/Pi/K1/K2/CC1 cross toolchains' libstdc++), catching a construct the CI runner's newer clang accepts and an older libstdc++ rejects (`std::from_chars<float>`, absent before GCC 11) before it reaches a cross build or a device. Runs in `.github/workflows/nightly.yml`'s `build` job, right after the native build produces the compile database; a failure fails the nightly run through `nightly-status`, without skipping the jobs that need `build`. `--compiler`, `--jobs`, or specific files to narrow it |
...
| `teardown-worktree.sh` | The inverse of `setup-worktree.sh`: remove a finished worktree and, by default, the branch it held. `git worktree remove` refuses outright on these trees ("working trees containing submodules cannot be moved or removed") because of the private `lvgl`/`libhv`/`helix-xml` checkouts, so removal is a guarded `rm -rf` plus a prune - which is safe only because `rm -rf` deletes a `lib/` symlink and never its target. Refuses to discard anything unique: uncommitted changes, unpushed commits in `lib/helix-xml` (ours, edited directly, unlike the patch-derived dirt in `lvgl`/`libhv`), a branch not contained in `--into` (default `main`), a path git does not list as a worktree of this repo, or a live build/test/git process with its cwd inside. Releases the tree's `helix-claim`, which would otherwise read LIVE against a directory that no longer exists. `-n` dry-runs; `--keep-branch`, `--force`, `--force-branch`. When `git branch -d` refuses a branch this script has confirmed merged, the cause is `-d` also checking the branch's UPSTREAM - the work is on local `main` but `main` is unpushed |
...
| `remote-build.sh` | Build on a remote host without sending the working tree. The remote keeps its own clone and fetches committed history from GitHub itself; the only thing crossing YOUR link is `git diff <newest-commit-origin-has>` (every local commit AND every uncommitted edit, as one patch) plus a tar of untracked files - a few KB, over one multiplexed SSH connection. `make remote-sync` stays for the Docker cross targets, which need a full mirror: rsync is delta-based but still exchanges metadata for every file under `lib/` and `assets/` before deciding nothing changed, and a fresh destination directory transfers ~260 MB. Backs `make remote-native` and `make remote-test`; `--run '<tag>'`, `--host`, `--dir`. Resets the remote clone with `reset --hard` each run, so it holds nothing but what this script puts there |
...
| Script | Purpose |
|--------|---------|
| `cloud/env-setup.sh` | Claude Code cloud environment setup script: runs once, as root, before the repo is cloned, then the filesystem is snapshotted. Installs build deps, seeds a full clone for submodule alternates, prebuilds a Python venv. It installs and configures ccache but fetches no prebuilt one - see BUILD_SYSTEM.md for the measurement behind that. `--deps-only` restricts it to the apt step and propagates its exit status for a CI caller. See `docs/devel/BUILD_SYSTEM.md` § "Cloud sessions: the warm environment" |
