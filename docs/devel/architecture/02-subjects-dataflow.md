...
Moonraker pushes status as JSON-RPC notifications (`notify_status_update` carrying changed Klipper status objects). The WebSocket frame lands on the libhv event-loop thread inside `MoonrakerClient`; HelixScreen's registered callback ([`src/application/moonraker_manager.cpp#register_callbacks`](../../../src/application/moonraker_manager.cpp#L633)) does exactly one thing and returns:
...
(verbatim from [`src/application/moonraker_manager.cpp#register_callbacks`](../../../src/application/moonraker_manager.cpp#L633); the `alive` flag is the manager's destruction guard). Connection-state changes ride the same queue as synthesized `_connection_state` marker objects (`src/application/moonraker_manager.cpp#register_callbacks/"m_notification_queue.push(state_change)"`). Nothing on that thread touches LVGL.

The dequeue happens on the main thread. `Application::main_loop()` ([`src/application/application.cpp#main_loop`](../../../src/application/application.cpp#L4081), entered once from `run()`) iterates: heartbeat and input housekeeping, `check_timeouts()`, then `process_notifications()` ([`src/application/application.cpp#main_loop`](../../../src/application/application.cpp#L4226)), and only then `lv_timer_handler()` (`src/application/application.cpp#main_loop/"time_till_next = lv_timer_handler()"`) where rendering happens. `MoonrakerManager::process_notifications()` ([`src/application/moonraker_manager.cpp#process_notifications`](../../../src/application/moonraker_manager.cpp#L254)) drains the whole queue each pass: `_connection_state` markers go to `PrinterState::set_printer_connection_state()`, while `notify_status_update` frames are unpacked — Klipper `eventtime` and the cached-snapshot marker are read out alongside the status object — and handed to the state model:
...
Why not `lv_async_call()`? It is not safe to call from another thread: it creates a one-shot LVGL timer (`lib/lvgl/src/misc/lv_async.c#lv_async_call`), and `lv_timer_create()` links that timer into LVGL's list without taking the LVGL lock (`lib/lvgl/src/misc/lv_timer.c#lv_timer_create`), so a background thread calling it races the main thread's timer walk. `queue_update()` only takes the queue's own mutex and leaves every LVGL call to the drain. The drain's safety property is serialization: callbacks run on the main thread inside the timer walk, strictly serialized with rendering and with every other timer, so a queued `lv_subject_set_*()` can never interleave with an in-progress render, which is what prevents LVGL's "invalidate during rendering" assertion. Ordering is not a guarantee. LVGL 9 timers have no priority field, `lv_timer_create()` inserts each new timer at the head of the list, the walk starts at the head, and it restarts whenever a timer is created or deleted, so whether the drain or the display refresh runs first in a pass depends on timer creation history: the refresh timer is created at display creation, before `update_queue_init()` ([`src/application/display_manager.cpp#init`](../../../src/application/display_manager.cpp#L397)), and a recreated display gets a newer one. The real guarantee is same-thread serialization plus the refresh period landing work within a frame.
...
- `helix::ui::run_on_main(tag, fn)` (`include/ui_update_queue.h#run_on_main`) runs inline when already on the main thread and queues otherwise — put it at a subsystem's public boundary rather than wrapping each internal call site (#960).

At audit time `queue_update(` appears at 302 call sites under `src/` and `include/`; `helix::ui::async_call()` is the drop-in same-signature replacement for `lv_async_call()` (`include/ui_update_queue.h#"async_call(lv_async_cb_t async_xcb, void* user_data,"`). PrinterState's setters are the canonical pattern the AGENTS.md threading rules point at: background code never writes a subject, it queues the write.

### Subject lifecycle: init, register, reset, deinit

Subjects exist so bindings can resolve at creation time (chapter 01), so the order at boot is fixed: register XML components ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L751)), initialize subjects (`src/application/application.cpp#run/"init_core_subjects()"`), and only then `lv_xml_create(m_screen, "app_layout", nullptr)` (`src/application/application.cpp#init_ui/"lv_xml_create"`). `SubjectInitializer` ([`src/application/subject_initializer.cpp`](../../../src/application/subject_initializer.cpp)) sequences the sweep in dependency phases — core globals → `PrinterState` → AMS/filament → navigation (registered after PrinterState precisely so reverse-order deinit clears its observers first) → panels → observers last (`init_core_and_state()` through `init_post()`). A soft restart for printer switching re-runs the same sequence; `Application::init_printer_state()` re-arms the UpdateQueue before subject init so background callbacks already have a working queue ([`src/application/application.cpp#tear_down_printer_state`](../../../src/application/application.cpp#L4974)).

Most subjects are initialized through the macros in [`include/state/subject_macros.h`](../../../include/state/subject_macros.h): `INIT_SUBJECT_INT(name, default, subjects, register_xml)` (`include/state/subject_macros.h#INIT_SUBJECT_INT`) initializes the `name_` member, registers it with the owning `SubjectManager` for cleanup, and registers it with the XML engine via `helix::xml::register_subject_in_current_scope()`. `INIT_SUBJECT_STRING` (`include/state/subject_macros.h#INIT_SUBJECT_STRING`) is the same for string subjects, which is where the naming convention bites: the macro expects a `char name_buf_[]` member, because LVGL string subjects **copy into a caller-owned fixed buffer** — the subject does not own its storage. A subject whose XML name is not its member name cannot use the macros at all, and registers at the call site instead: [`src/printer/printer_temperature_state.cpp#init_subjects`](../../../src/printer/printer_temperature_state.cpp) publishes `extruder_temp`/`extruder_target` from `active_extruder_temp_`/`active_extruder_target_` that way. The XML name goes to `SubjectManager::register_subject()` as well, so teardown can withdraw it — the macros pass it for the same reason. Tree-wide at audit: 184 `INIT_SUBJECT_*` invocation sites across 17 files under `src/`, plus 226 direct `lv_xml_register_subject()` call sites — the old diagram's "50+ more subjects" was off by roughly an order of magnitude.
...
1. [`src/application/moonraker_manager.cpp#register_callbacks`](../../../src/application/moonraker_manager.cpp#L632) — `register_notify_update`: the entire background-thread obligation, one mutex-protected push. Then walk down to `src/application/moonraker_manager.cpp#process_notifications` `process_notifications()` and notice the two payload shapes (`_connection_state` marker vs `notify_status_update`) and the direct `update_from_status()` call at `src/application/moonraker_manager.cpp#process_notifications/"get_printer_state().update_from_status"`.
2. [`src/application/application.cpp#main_loop`](../../../src/application/application.cpp#L4223) — the main-loop strip from `check_timeouts()` through `process_notifications()` (`src/application/application.cpp#main_loop/"process_notifications()"`) to `lv_timer_handler()` (`src/application/application.cpp#main_loop/"time_till_next = lv_timer_handler()"`). This is the ordering the mermaid diagram encodes.
3. [`src/printer/printer_state.cpp#update_from_notification`](../../../src/printer/printer_state.cpp#L389) — `update_from_status()`: the `state_mutex_` lock and the fan-out to state components. Then `src/printer/printer_state.cpp#set_printer_connection_state` and `src/printer/printer_state.cpp#set_klippy_state` for the two marshalling flavors (`async_lifetime_.defer` vs `helix::async::call_method`), and `src/printer/printer_state.cpp#set_klippy_state_internal` `set_klippy_state_internal()` — the chokepoint that also resets Klippy-volatile subjects.
4. [`src/printer/printer_network_state.cpp#set_klippy_state_internal`](../../../src/printer/printer_network_state.cpp#L123) — a complete setter: change detection, `lv_subject_set_int()`, side effects only on transition.
5. [`src/printer/printer_temperature_state.cpp#update_from_status`](../../../src/printer/printer_temperature_state.cpp#L451) — the decidegrees convention, the explicit `lv_subject_notify()` on equal values, and (at `src/printer/printer_temperature_state.cpp#init_subjects`) the call-site registration the two active-extruder subjects need because their XML names differ from their members.
...
7. [`include/ui_update_queue.h#helix::ui`](../../../include/ui_update_queue.h#L94) — the `UpdateQueue` class: timer creation at `include/ui_update_queue.h#UpdateQueue/init`, tagged `process_pending()` at `include/ui_update_queue.h#UpdateQueue/process_pending`, `ScopedFreeze` at `include/ui_update_queue.h#UpdateQueue/"class ScopedFreeze"`, and the widget-safe overloads at the bottom of the file.
