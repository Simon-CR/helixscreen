...
| Shared styles in the theme table | 40 | `StyleRole` entries before `COUNT` ([`include/theme_manager.h#StyleRole`](../../../include/theme_manager.h#L71)) |
| `theme_manager_get_color()` call sites | 409 | `rg -c 'theme_manager_get_color\(' src include \| awk -F: '{s+=$2} END {print s}'` |
| Breakpoint tiers | 7 | `UiBreakpoint` ([`include/ui_breakpoint.h#UiBreakpoint`](../../../include/ui_breakpoint.h#L26)): Micro..XXLarge |
| Layout variants | 7 | `LayoutType` ([`include/layout_manager.h#LayoutType`](../../../include/layout_manager.h#L10)): standard, ultrawide, portrait, micro, micro_portrait, tiny, tiny_portrait |
| Variant dirs with files | 2 | `find ui_xml -maxdepth 1 -type d`: `micro/` (4 files), `portrait/` (2 files); `micro_portrait/` exists empty |
...
1. globals.xml declares the ladder: `space_md_tiny=6`, `space_md_small=8`, `space_md_medium=10`, `space_md_large=12`, `space_md_xlarge=16`, `space_md_xxlarge=20` ([`ui_xml/globals.xml#space_md_tiny`](../../../ui_xml/globals.xml#L65)). No bare `space_md` exists anywhere in XML — the base name is C++-registered only.
...
Where `ui_xml/` is installed rather than edited, tokens are compiled in: [`src/generated/theme_token_table.cpp`](../../../src/generated/theme_token_table.cpp) (regenerate with `make regen-tokens` from [`scripts/gen_theme_tokens.py`](../../../scripts/gen_theme_tokens.py)) is the table, enabled by [`theme_token_table_runtime.cpp`](../../../src/ui/theme_token_table_runtime.cpp) for ESP32 and for cross-built release targets (`HELIX_RELEASE_BUILD`), and a unit test fails the build if the compiled table drifts from what the runtime scanner would parse. Native dev builds parse XML live — editing [`ui_xml/globals.xml`](../../../ui_xml/globals.xml) and relaunching adjusts tokens without a rebuild; `HELIX_TOKEN_TABLE=0` restores that on a device, and `=1` previews the table anywhere.

### The theme engine: shared styles + the `helix_theme` wrapper

A theme is a JSON file with `dark` and/or `light` 16-color `ModePalette`s plus non-color properties (border radius/width, shadow). The 16 slots are semantic and fixed — `screen_bg`, `overlay_bg`, `card_bg`, `elevated_bg`, `border`, `text`, `text_muted`, `text_subtle`, `primary`, `secondary`, `tertiary`, `info`, `success`, `warning`, `danger`, `focus` ([`include/theme_loader.h#focus`](../../../include/theme_loader.h#L54)) — which is exactly the set `theme_manager_get_color()` can resolve. A theme cannot invent new color names; it fills the slots. [`theme_loader.cpp`](../../../src/ui/theme_loader.cpp) loads by name — the user's config dir `themes/` first, then `assets/config/themes/defaults/`. The active name comes from `/display/theme` in settings.json, overridable with the `HELIX_THEME` env var for testing and screenshots ([`src/ui/theme_manager.cpp`](../../../src/ui/theme_manager.cpp)). Single-mode themes are legal; `ThemeModeSupport` records which.
...
- **The `helix_theme` wrapper** ([`src/ui/theme_manager.cpp#helix_theme`](../../../src/ui/theme_manager.cpp#L210)) is the bridge into LVGL's own theming. Its apply callback (`helix_theme_apply`, [`src/ui/theme_manager.cpp#"static void helix_theme_apply(lv_theme_t* theme, lv_obj_t* obj) {"`](../../../src/ui/theme_manager.cpp#L563)) runs for every widget created: first it calls through to `lv_theme_default_init()`'s theme for base padding and tracks, then layers the appropriate shared styles per widget class — buttons get `Button` + `Pressed` + `Focused`, textareas get `InputBg`, sliders get track/indicator/knob styles, dropdowns get contrast-selected list styling. `theme_manager_init()` (called from `Application`, [`src/application/application.cpp#init_theme`](../../../src/application/application.cpp#L1683)) wires it up with `lv_theme_set_apply_cb()`.

Live switching is the payoff, and `theme_manager_apply_theme(theme, dark)` is the single entry point ([`src/ui/theme_manager.cpp#theme_manager_apply_theme`](../../../src/ui/theme_manager.cpp#L2057)). The sequence, in order:
...
The user-facing controls live in Settings → Display: `DisplaySettingsManager` ([`src/system/display_settings_manager.cpp#set_dark_mode`](../../../src/system/display_settings_manager.cpp#L435)) guards against requesting a mode the theme lacks, persists `/dark_mode` and `/display/theme` to settings.json, and disables the toggle outright for single-mode themes (a dark-only theme also force-switches the mode and hides the control). It publishes `settings_dark_mode` and `settings_dark_mode_available` subjects for the settings UI to bind; `Application` calls its `on_theme_changed()` once after both theme and settings are up so the toggle reflects what the loaded theme actually supports ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L874)).
...
`PanelWidgetConfig::build_default_grid()` ([`src/system/panel_widget_config.cpp#build_default_grid`](../../../src/system/panel_widget_config.cpp#L742)) reads the file through `helix::find_readable()` at runtime — a user's config-dir copy wins over the shipped asset, so layouts are editable on-device without a rebuild. Widgets not listed in any table are auto-placed after the anchored ones.
...
- **Testing across sizes** — run with `-s/--size <preset>` (micro, tiny, small, medium, large, xlarge presets from [`include/theme_manager.h#UI_SCREEN_MICRO_W`](../../../include/theme_manager.h#L288)); combined with `--layout` this covers the geometry matrix without hardware.
...
- **The same ordering bites styles:** globals.xml is parsed *before* `theme_manager_init()` registers base `#space_*`/color consts, so a `<style>` in globals.xml referencing one registers empty (the property is silently skipped). Put such styles in `ui_xml/styles.xml` when more than one file uses them (borrowed by dotted name, `styles.<name>`), otherwise in the consuming component's own `<styles>` block; both parse after theme init. See [`UI_CONTRIBUTOR_GUIDE.md` § "Shared styles"](../UI_CONTRIBUTOR_GUIDE.md#shared-styles-ui_xmlstylesxml).
...
- **Never hand-roll breakpoint thresholds.** `if (res <= 460)` chains drift from the ladder; use `breakpoint_for(res)` / `responsive_pick()` so the tier boundaries live in exactly one place ([`include/ui_breakpoint.h#breakpoint_for`](../../../include/ui_breakpoint.h#L56)).
...
- **Deinit order:** `theme_manager_deinit()` must run before `lv_deinit()` or observer teardown crashes ([`include/theme_manager.h#theme_manager_deinit`](../../../include/theme_manager.h#L338)).
- **Overlay geometry is orientation-dependent and applied at push time.** Landscape puts the nav bar on the leading edge as a vertical strip; portrait puts it along the bottom ([`ui_xml/navigation_bar.xml`](../../../ui_xml/navigation_bar.xml), bound to `ui_is_portrait`). `ui_set_overlay_geometry()` ([`include/theme_manager.h#ui_set_overlay_geometry`](../../../include/theme_manager.h#L781)) is the sole writer — the overlay's width (or, in portrait, height and alignment) is computed from tokens when it is pushed, not baked into its XML.
...
1. [`include/theme_manager.h#StyleRole`](../../../include/theme_manager.h#L30) — the `StyleRole` enum; 40 roles, each a shared style. The file header states the threading and lookup rules.
2. [`src/ui/theme_manager_new.cpp#init`](../../../src/ui/theme_manager_new.cpp#L53) — `ThemeManager::init()`: registers style configs, applies default Nord palettes if none were set. Small file; read it whole.
3. [`src/ui/style_configs.cpp`](../../../src/ui/style_configs.cpp) — skim three or four `configure_*` functions to see how palette colors become style properties.
4. [`src/ui/theme_manager.cpp#"static void helix_theme_apply(lv_theme_t* theme, lv_obj_t* obj) {"`](../../../src/ui/theme_manager.cpp#L563) — `helix_theme_apply()`: default theme first, shared styles per widget class after. This is what "the custom theme" actually is today.
5. [`src/ui/theme_manager.cpp#theme_init_lvgl`](../../../src/ui/theme_manager.cpp#L745) — `theme_init_lvgl()`: builds both palettes, sets up `ThemeManager`, calls `lv_theme_default_init()` and layers `helix_theme` on top.
6. [`src/ui/theme_manager.cpp#theme_manager_resolve_px_tokens`](../../../src/ui/theme_manager.cpp#L1131) — `theme_manager_resolve_px_tokens()`: the two-axis suffix choice, the `nav_width` special case, the required-triplet rule.
7. [`src/ui/theme_manager.cpp#theme_manager_register_semantic_colors`](../../../src/ui/theme_manager.cpp#L1572) — `theme_manager_register_semantic_colors()`: theme JSON palette → `card_bg`/`card_bg_light`/`card_bg_dark` consts.
8. [`src/ui/theme_manager.cpp`](../../../src/ui/theme_manager.cpp) — `theme_manager_get_color()`: the `_light`/`_dark` probe order and the partial-variant error path.
9. [`src/ui/theme_manager.cpp#theme_manager_apply_theme`](../../../src/ui/theme_manager.cpp#L2057) — `theme_manager_apply_theme()`: the whole live-switch sequence from swap maps to generation bump.
10. [`include/ui_breakpoint.h#UiBreakpoint`](../../../include/ui_breakpoint.h#L26) — the `UiBreakpoint` ladder, the narrow-axis thresholds, and `breakpoint_for()`/`responsive_pick()`.
11. [`ui_xml/globals.xml#space_xxs_tiny`](../../../ui_xml/globals.xml#L44) — the `space_*` ladder in situ; note the comments documenting the register-before-parse ordering traps.
12. [`src/layout_manager.cpp#detect_layout_type`](../../../src/layout_manager.cpp#L111) — `detect_layout_type()`: aspect-ratio classification; then `variant_chain()` at [`src/layout_manager.cpp#variant_chain`](../../../src/layout_manager.cpp#L69).
13. [`assets/config/default_layout.json`](../../../assets/config/default_layout.json) — the anchor table and the `variants.portrait` override; the `_comment` fields document the fallback rules.
14. [`src/system/panel_widget_config.cpp#build_default_grid`](../../../src/system/panel_widget_config.cpp#L742) — `build_default_grid()` loading those anchors through `variant_chain()`.
15. [`src/ui/theme_loader.cpp#load_theme_from_file`](../../../src/ui/theme_loader.cpp#L297) — `load_theme_from_file()`: user-dir-first lookup, mode-support parsing.
16. [`src/system/display_settings_manager.cpp#validate_timeout_option`](../../../src/system/display_settings_manager.cpp#L154) — the settings side: `/dark_mode` read, subjects published, mode-support guards.
17. [`scripts/check_responsive_token_scope.py`](../../../scripts/check_responsive_token_scope.py) and [`scripts/check_variant_parity.py`](../../../scripts/check_variant_parity.py) — read both docstrings; they encode the two easiest ways to silently break this subsystem.
