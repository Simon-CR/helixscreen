...
flowchart TD
    ...
    subgraph LEDGER["The ledger — 367 sites (issue #1140)"]
        V["visibility 148"]
        T["text 93"]
        S["style 63"]
        E["event 67"]
...
### The imperative-UI ledger: 367 sites and the ratchet

[`scripts/check_imperative_ui.py`](../../../scripts/check_imperative_ui.py) flags one specific shape: a widget fetched from an XML tree with `lv_obj_find_by_name()` and then mutated in C++ — `lv_label_set_text()` instead of `bind_text`, `lv_obj_add_flag(HIDDEN)` instead of `<bind_flag_if_eq>`, `lv_obj_set_style_*` instead of XML styles, `lv_obj_add_event_cb()` instead of `<event_cb>`. At this audit the count is **367**:

```
  event          64   → <event_cb trigger="clicked" callback="name"/> + lv_xml_register_event_cb()
  style          63   → XML style attribute (style_bg_color="#card_bg") or bind_style
  text           93   → bind_text="subject" on the XML element
  visibility    147   → <bind_flag_if_eq subject=... flag="hidden"> or <if cond=...>
  TOTAL         367
```
...
| Area | Sites | Notes |
|------|-------|-------|
| `src/ui/` flat files | 323 | panels, overlays, wizards, services |
