...
flowchart TB
    ...
    CB["HomePanel rebuild callback<br/>ui_panel_home.cpp#setup_widget_gate_observers<br/>skips during grid edit mode or<br/>while any widget has_overlay_open()"]
...
`PanelWidget` ([`include/panel_widget.h#PanelWidget`](../../../include/panel_widget.h#L32)) is a small interface with one load-bearing idea: the C++ object outlives the LVGL objects it wires. Hooks, in call order:

- `init_subjects()` — create LVGL subjects *before* `lv_xml_create()`; registered per-widget-def as a `SubjectInitFn` in the registry and fired once by `PanelWidgetManager::init_widget_subjects()` (idempotent, [`src/ui/panel_widget_manager.cpp#init_widget_subjects`](../../../src/ui/panel_widget_manager.cpp#L161)).
- `set_config(json)` — per-widget config from the saved layout, after factory creation, before `get_component_name()` / `attach()`.
- `get_component_name()` — defaults to `panel_widget_<id>`; override to pick between XML layouts (fan stack vs carousel, [`src/ui/panel_widgets/fan_stack_widget.cpp#get_component_name`](../../../src/ui/panel_widgets/fan_stack_widget.cpp#L132)).
...
`PanelWidgetManager::populate_widgets(panel_id, container, page_index, reuse)` ([`src/ui/panel_widget_manager.cpp#populate_widgets`](../../../src/ui/panel_widget_manager.cpp#L210)) is the single build path; `HomePanel::populate_page()` is its only caller of note.

1. **Resolve slots.** For each enabled entry in the page's `PanelWidgetConfig`, look up the def, check its hardware gate subject (`gate && lv_subject_get_int(gate) == 0` → widget renders dimmed at 40% opacity with a slash badge, but stays placed). Acquire the C++ instance: reuse map first, then `def->factory(entry.id)`; either way `set_panel_id` + `set_config` + `get_component_name` follow. Each widget is built inside its own try/catch — a malformed config or throwing factory skips one tile, not the dashboard (`src/ui/panel_widget_manager.cpp#"configuration failed"`).
2. **Short-circuit.** Compare the ordered, gate-suffixed widget ID list (`"fan_stack"` vs `"fan_stack~gated"`) against the cached one from the previous populate; if identical and the container still has children, return without touching anything (`src/ui/panel_widget_manager.cpp#"Widget list unchanged"`). Gate state is part of the key precisely so a gated→ungated transition rebuilds and attaches the instance.
3. **Place.** Anchored widgets (explicit col/row in config) are placed first with their spans clamped to the grid; the rest are auto-placed by `GridLayout` — authored spans first, and if not everyone fits, a minimum-first pass that grows survivors into leftover cells (`run_auto_pass`, `src/ui/panel_widget_manager.cpp#"run_auto_pass = [&]"`). A widget that cannot fit the grid *at all* is disabled with a notification; a widget that fits but finds no free cell is only evicted from its position and stays enabled (`disable_unplaceable` vs `evict_for_full_grid`, `src/ui/panel_widget_manager.cpp#"disable_unplaceable = [&]"` and `src/ui/panel_widget_manager.cpp#"evict_for_full_grid = [&]"` — the distinction and its history are #1216's). Positions are written back and persisted, but never a span that was shrunk to fit this screen, and never a layout computed while a connected printer reports Klipper not READY — that arrangement is not the user's, and freezing it is what made tiles revert after a power cycle.
4. **Create and attach.** Card backgrounds are laid first (merged rectangles behind adjacent 1×1 tiles), then per placed widget: `lv_xml_create(container, component_name)`, `lv_obj_set_grid_cell`, `lv_obj_set_name(widget_id)` — that name is how grid edit mode identifies tiles — `lv_obj_add_flag(widget, PANEL_WIDGET_TILE_FLAG)`, and finally `attach()` + `on_size_changed()` unless the widget is gated. The grid layout itself is activated *last*, after all children exist: a child whose `attach()` synchronously forces layout over a half-built grid walked off a freed descriptor and SIGSEGV'd (#983, `src/ui/panel_widget_manager.cpp#"lv_obj_set_layout(container, LV_LAYOUT_GRID);"`).

`HomePanel` owns the page dimension: a carousel of per-page containers (`build_carousel`, [`src/ui/ui_panel_home.cpp#build_carousel`](../../../src/ui/ui_panel_home.cpp#L283)), each populated independently, with `on_activate()`/`on_deactivate()` fanned to the active page's widgets on page change (`src/ui/ui_panel_home.cpp#on_page_changed`).

### Rebuilds: gate observers, the reuse map, and why the camera survives

Three forces rebuild the grid, all converging on `HomePanel::populate_page()`. Gate observers, config changes and an edit-mode rearrange reach it for every page through `HomePanel::populate_widgets()` ([`src/ui/ui_panel_home.cpp#populate_widgets`](../../../src/ui/ui_panel_home.cpp#L708)); a change to the page set rebuilds the whole carousel instead, and the carousel build populates every page of the new carousel (`src/ui/ui_panel_home.cpp#rebuild_carousel` -> `src/ui/ui_panel_home.cpp#build_carousel`):

- **Gate observers.** `setup_gate_observers(panel_id, rebuild_cb)` ([`src/ui/panel_widget_manager.cpp#setup_gate_observers`](../../../src/ui/panel_widget_manager.cpp#L1302)) subscribes, per panel, to every distinct `hardware_gate_subject` in the registry — so a newly gated widget type (or any future one) is picked up without editing the manager. A firing does not rebuild inline: the observer lambda sets a `pending` flag on a stable per-panel `GateRebuildSlot` and queues exactly one `lv_async_call` trampoline (`src/ui/panel_widget_manager.cpp#"s.pending = true;"`). Coalescing matters twice over — N gates flipping in one tick used to produce N `populate_page` calls whose accumulated async deletes corrupted LVGL's event list on memory-tight hardware (L081 family), and `lv_async_call` is what escapes the UpdateQueue batch (chapter 03). The HomePanel callback adds two guards before rebuilding: skip while grid edit mode is active (its overlay pointers would be cleaned mid-edit), and skip while any widget returns `has_overlay_open()` ([`src/ui/ui_panel_home.cpp#setup_widget_gate_observers`](../../../src/ui/ui_panel_home.cpp#L724)-742).
...
- **Edit mode / pages.** A rearrange in edit mode rebuilds through `GridEditMode`'s rebuild callback, which runs `populate_widgets()` on the next tick, outside the input dispatch that committed it. A change to the page set rebuilds the whole carousel instead, also on the next tick: a drop that creates a page, and a move or a removal that takes the last widget of a page other than the main page (which removes that page), reach `HomePanel::on_edit_pages_changed` through the grid's pages-changed callback, and page deletion schedules it itself, so every one lands where `helix::page_set_landing()` puts it: on the page that was on screen, with one slide to the change's focus when that is another page. [`../HOME_EDIT_MODE.md`](../HOME_EDIT_MODE.md) has the whole page lifecycle.

The reuse dance happens in `HomePanel::populate_page()` ([`src/ui/ui_panel_home.cpp#reuse`](../../../src/ui/ui_panel_home.cpp#L457)-492): `detach()` every active widget on the page; ones returning `supports_reuse()` move into a `WidgetReuseMap` (id → `unique_ptr<PanelWidget>`); the LVGL tree is cleaned via `safe_clean_children` under an UpdateQueue freeze+drain (gate observers arrive through the queue — a synchronous `lv_obj_clean` batched with sibling deletes corrupts the event list, #776/#834); then `populate_widgets()` receives the map, and each widget either gets its old instance re-`attach()`ed to the fresh tree ("Reusing widget instance" in the debug log, [`src/ui/panel_widget_manager.cpp#populate_widgets`](../../../src/ui/panel_widget_manager.cpp#L273)) or built from its factory.

The contract that makes this safe ([`include/panel_widget.h#PanelWidget`](../../../include/panel_widget.h#L87)-94): `detach()` clears LVGL pointers and observers only; the destructor does full cleanup; `attach()` must work on a previously detached instance. The motivating case is `CameraWidget` ([`src/ui/panel_widgets/camera_widget.cpp#detach`](../../../src/ui/panel_widgets/camera_widget.cpp#L168)-181): its MJPEG stream thread keeps running across rebuilds, and frame callbacks arriving in the detach→reattach gap find `camera_image_ == nullptr` and no-op — pointers die, state doesn't. Background threads surviving the gap is exactly the chapter 03 lifetime discipline; alive guards stay valid across `detach()` by design.
...
- `FanStackWidget`: `setup_common_observers()` subscribes to `printer_state_.get_fans_version_subject()`; each bump calls `bind_fans()` ([`src/ui/panel_widgets/fan_stack_widget.cpp#bind_fans`](../../../src/ui/panel_widgets/fan_stack_widget.cpp#L515)), which re-classifies the primary part/hotend/aux fans and rebinds one speed observer per fan via `bind_fan_observer()` (`src/ui/panel_widgets/fan_stack_widget.cpp#bind_fan_observer`) — `SubjectLifetime` + the widget's lifetime token, the observer-factory pattern from chapter 02.
- `LedWidget`: observes `LedController`'s `led_config_version` in `attach()`; each bump calls `bind_led()` against the currently selected strips ([`src/ui/panel_widgets/led_widget.cpp#attach`](../../../src/ui/panel_widgets/led_widget.cpp#L75)).
- `PowerDeviceWidget`: the same shape keyed on `power_device_count` instead of a version counter — the count itself changes on discovery ([`src/ui/widgets/power_device_widget.cpp#attach`](../../../src/ui/widgets/power_device_widget.cpp#L167)).

Two subtleties both widget families hit. First, `observe_int_sync` defers its initial fire-on-add through `queue_update` — and `populate_widgets()` runs under an UpdateQueue freeze, so that initial fire can be dropped entirely. Widgets therefore read the current value explicitly after binding: `bind_fan_observer()` calls `on_update(lv_subject_get_int(subject))` itself ([`src/ui/panel_widgets/fan_stack_widget.cpp#bind_fan_observer`](../../../src/ui/panel_widgets/fan_stack_widget.cpp#L843)-845), and `LedWidget::attach()` calls `bind_led()` directly rather than waiting ([`src/ui/panel_widgets/led_widget.cpp#attach`](../../../src/ui/panel_widgets/led_widget.cpp#L82)-86). Second, every observer callback must re-check the lifetime token (`token.expired()`) before touching `this` — detach invalidates the token, and a queued callback may be the very thing being drained.
...
- **Never re-read gate subjects after populate and cache the result** — late-arriving capability flips get baked into the cache and the next rebuild short-circuits with the widget stuck gated. `HomePanel` snapshots IDs once at entry for exactly this reason ([`src/ui/ui_panel_home.cpp#populate_page`](../../../src/ui/ui_panel_home.cpp#L436)-454).
...
- **Renaming or re-polarizing a per-widget config key needs two migrations, not one.** The parser's own tolerant fallback (`favorite_macro_config_from_json`) covers configs that never reach a versioned migration — preset assets under `assets/config/panel_widgets/<preset>/`, hand-edited files, an imported widget config — while a `migrate_vN_to_vN+1` in [`src/system/config.cpp`](../../../src/system/config.cpp) sheds the old key from the user's settings so it does not linger forever. Neither is redundant: the versioned pass cannot see preset assets, and the parser fallback never writes. `migrate_v22_to_v23` is the worked example, and it shows the shape of the walk — per-widget config is buried four levels deep under `printers/<id>/panel_widgets/<panel>/pages[]/widgets[]/config`, so the migration has to descend it by hand.
...
- [`../HOME_EDIT_MODE.md`](../HOME_EDIT_MODE.md) - edit mode itself: the session over a page, the grab and click rules, the events that end a gesture, cross-page drag and the page lifecycle.
...
1. [`include/panel_widget.h#helix`](../../../include/panel_widget.h#L14) — the `PANEL_WIDGET_TILE_FLAG` doc comment and the user-flag ledger, then the class itself (`include/panel_widget.h#PanelWidget`): read the hooks in order, and note which say "called before XML" vs "after".
2. [`include/panel_widget_registry.h#PanelWidgetDef`](../../../include/panel_widget_registry.h#L46) — `PanelWidgetDef`: every field is a decision the manager or the catalog makes somewhere.
3. [`src/ui/panel_widget_registry.cpp#helix`](../../../src/ui/panel_widget_registry.cpp#L88) — the def table itself: spot the gate subjects, the span triples (min/default/max), the `multi_instance` column, and the `HELIX_HAS_CAMERA` guard; then `init_widget_registrations()` (`src/ui/panel_widget_registry.cpp#init_widget_registrations`) and the `:N` suffix strip in `find_widget_def()` (`src/ui/panel_widget_registry.cpp#find_widget_def`).
4. [`src/ui/panel_widget_manager.cpp#populate_widgets`](../../../src/ui/panel_widget_manager.cpp#L210) — `populate_widgets()` top to bottom once: slot resolution and the per-widget try/catch (`src/ui/panel_widget_manager.cpp#"configuration failed"`), the `~gated` short-circuit key (`src/ui/panel_widget_manager.cpp#"Widget list unchanged"`), `safe_clean_children` (`src/ui/panel_widget_manager.cpp#"safe_clean_children(container);"`) + `LV_LAYOUT_NONE` (`src/ui/panel_widget_manager.cpp#"lv_obj_set_layout(container, LV_LAYOUT_NONE);"`), the two auto-place policies (`src/ui/panel_widget_manager.cpp#"run_auto_pass = [&]"`), the write-back rules (`src/ui/panel_widget_manager.cpp#"Never persist a layout computed"`), and the create/attach loop with the tile flag (`src/ui/panel_widget_manager.cpp#"lv_obj_add_flag(widget, PANEL_WIDGET_TILE_FLAG);"`) and grid-last activation (`src/ui/panel_widget_manager.cpp#"lv_obj_set_layout(container, LV_LAYOUT_GRID);"`).
5. [`src/ui/panel_widget_manager.cpp#setup_gate_observers`](../../../src/ui/panel_widget_manager.cpp#L1302) — `setup_gate_observers()`: the registry walk for distinct gate subjects, the `GateRebuildSlot` coalescing (queue once, `pending` until the trampoline runs), and `clear_gate_observers()` canceling in-flight rebuilds (`src/ui/panel_widget_manager.cpp#clear_gate_observers`).
6. [`src/ui/ui_panel_home.cpp#populate_page`](../../../src/ui/ui_panel_home.cpp#L415) — `populate_page()`: the ID snapshot, reuse-map extraction (`src/ui/ui_panel_home.cpp#reuse`), freeze+drain+`safe_clean_children` (`src/ui/ui_panel_home.cpp#"safe_clean_children(container)"`), and the gate-observer registration with its edit-mode and overlay guards (`src/ui/ui_panel_home.cpp#setup_widget_gate_observers`).
7. [`include/panel_widget_config.h#PanelWidgetEntry`](../../../include/panel_widget_config.h#L17) — `PanelWidgetEntry` (`has_grid_position`, col/row/span, per-widget config JSON) and the `PanelWidgetConfig` API around it: pages, `set_widget_config`, and the preset-seed loader (`include/panel_widget_config.h#try_populate_from_preset_seed`).
8. [`src/ui/panel_widgets/motion_widget.cpp#register_motion_widget`](../../../src/ui/panel_widgets/motion_widget.cpp#L16) — the whole 64-line widget: factory, XML callback registration, `attach`/`detach`, a lazy overlay open. This is the minimum viable `PanelWidget`.
9. [`src/ui/panel_widgets/fan_stack_widget.cpp#register_fan_stack_widget`](../../../src/ui/panel_widgets/fan_stack_widget.cpp#L39) — the factory + callback registration; then `attach_stack` (`src/ui/panel_widgets/fan_stack_widget.cpp#attach_stack`) (cache pointers by name, then bind), `bind_fans` (`src/ui/panel_widgets/fan_stack_widget.cpp#bind_fans`), `bind_fan_observer` (`src/ui/panel_widgets/fan_stack_widget.cpp#bind_fan_observer`) (note the manual initial-value read, `src/ui/panel_widgets/fan_stack_widget.cpp#"on_update(lv_subject_get_int(subject));"`), and the version observer in `setup_common_observers` (`src/ui/panel_widgets/fan_stack_widget.cpp#setup_common_observers`).
10. [`src/ui/panel_widgets/led_widget.cpp#attach`](../../../src/ui/panel_widgets/led_widget.cpp#L46) — `attach()` through `bind_led()` (`src/ui/panel_widgets/led_widget.cpp#bind_led`): the second instance of the version-observer pattern, plus the comment (`src/ui/panel_widgets/led_widget.cpp#"ui_button allocates its own UiButtonData"`) about `ui_button` owning its `user_data` — why `lv_obj_set_user_data` on a button is a bug.
11. [`src/ui/panel_widgets/camera_widget.cpp#detach`](../../../src/ui/panel_widgets/camera_widget.cpp#L168) — `detach()`: the lightweight contract in action — observers and pointers out, stream alive; frame callbacks no-op on null until re-attach. Glance at `src/ui/panel_widgets/camera_widget.cpp#"leaking CameraStream to avoid UAF"` for the detached-thread leak trade-off (#624).
12. [`src/ui/widgets/power_device_widget.cpp#attach`](../../../src/ui/widgets/power_device_widget.cpp#L167) — the count-subject variant of self-binding, and a widget whose implementation lives outside `panel_widgets/`.
13. [`src/ui/panel_widgets/heater_temp_widget.cpp#register_temperature_widget`](../../../src/ui/panel_widgets/heater_temp_widget.cpp#L75) — one class, three defs: the shared `register_heater_temp_widget()` helper and config-selected instantiation.
14. [`include/grid_edit_mode.h`](../../../include/grid_edit_mode.h) - the other half of the layout story: how tiles get dragged, and the rebuild and pages-changed callbacks that end a committed rearrange. End here; the grid internals are [`../LAYOUT_SYSTEM.md`](../LAYOUT_SYSTEM.md), and edit mode's gestures and pages are [`../HOME_EDIT_MODE.md`](../HOME_EDIT_MODE.md).
